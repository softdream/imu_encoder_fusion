...
public:
	...
	void update( const Vector3 &z ) // theta, s, v
	{
		...
		DataType tmp = ::cos( x_pre[0] + 0.5 * h_measurement[0] ) + ::sin( x_pre[0] + 0.5 * h_measurement[0] ) ;
		...
		Matrix3x5 H;
		H <<	1, 							0,                 0,              0,       0;
		     ( ::sin( x_pre[0] + 0.5 * h_measurement[0] )
		       -::cos( x_pre[0] + 0.5 * h_measurement[0] ) ) 
		       * ( x_now[3] - x_pre[3] + x_now[4] - x_pre[4] ) * 0.5,   0,                 0,             1 / tmp, 1 / tmp,
			0,     		                                      1 / h_measurement, 1 / h_measurement, 0,       0;
		// Kalman gain
		Matrix5 K_tmp = H * P_now * H.transpose() + R;
